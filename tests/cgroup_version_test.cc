...
using mydocker::parse_mount_info;
...
TEST(CgroupVersionTest, ParseMountInfoLine_V1) {
  ...
  EXPECT_EQ(parse_mount_info(line), CgroupVersion::kV1);
}
...
TEST(CgroupVersionTest, ParseMountInfoLine_V2) {
  ...
  EXPECT_EQ(parse_mount_info(line), CgroupVersion::kV2);
}
...
TEST(CgroupVersionTest, ParseMountInfoLine_UnknownFsType) {
  ...
  EXPECT_EQ(parse_mount_info(line), CgroupVersion::kUnknown);
}
